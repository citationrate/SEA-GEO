...
export function Sidebar({ profile }: SidebarProps) {
  ...
  return (
    <aside className="w-56 flex-shrink-0 flex flex-col bg-ink-2 border-r border-border">
      {/* Logo */}
      <div className="h-14 flex items-center px-4 border-b border-border flex-shrink-0">
        <Link href="/dashboard" className="flex items-center gap-2.5 group">
          <div className="w-7 h-7 rounded-sm bg-sage/10 border border-sage-dim flex items-center justify-center transition-all group-hover:glow-primary">
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-2.5 space-y-4">
        {NAV.map(({ group, items }) => (
          <div key={group}>
            <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-cream-dim px-2 mb-1.5">
              {group}
            </p>
              ...
              {items.map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className={cn(
                      "flex items-center gap-2.5 px-2 py-1.5 rounded-sm text-sm font-sans transition-all duration-100",
                      isActive(item.href)
                        ? "bg-sage/10 text-sage font-medium"
                        : "text-cream-dim hover:text-foreground hover:bg-ink-3"
                    )}
                  >
                    <item.icon className={cn(
                      "w-[15px] h-[15px] flex-shrink-0",
                      isActive(item.href) ? "text-sage" : "text-cream-dim"
                    )} />
                    <span className="flex-1">{item.label}</span>
                    {isActive(item.href) && (
                      <ChevronRight className="w-3 h-3 text-sage/50" />
                    )}
                  </Link>
                </li>
              ))}
      ...
      <div className="border-t border-border p-3 flex-shrink-0">
        <div className="flex items-center gap-2.5 px-2 py-1.5 rounded-sm hover:bg-ink-3 transition-colors cursor-pointer">
          <div className="w-6 h-6 rounded-sm bg-sage/15 flex items-center justify-center flex-shrink-0 text-sage font-mono text-xs">
          ...
          <div className="min-w-0">
            ...
            <p className="font-mono text-[10px] text-cream-dim uppercase tracking-wide">

...
function buildVars(type: EmailType, data: any): Record<string, string | number | null> {
  ...
  return {
    ...
    aviScore: data.avi_score != null ? String(Math.round(Number(data.avi_score))) : "",
    presence: data.presence_score != null ? String(Math.round(Number(data.presence_score))) : "",
    sentiment: data.sentiment_score != null ? String(Math.round(Number(data.sentiment_score))) : "",
    avgRank: data.avg_brand_rank != null ? Number(data.avg_brand_rank).toFixed(1) : "",
    plan: data.plan || "",

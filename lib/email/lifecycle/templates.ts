/**
 * Lifecycle email templates — Bilingual IT + EN.
 * 12 funzioni: W0 (welcome) + 1A/1B/1C (recovery one-shot) + D1/D2/D3/D4_CS/D4_AVI/D5_CS/D5_AVI/D6 (drip).
 * Ogni funzione → { subject, html }.
 * Ogni email contiene prima la versione italiana, poi un divider, poi la versione inglese.
 */
...
function bilingualDivider(): string {
  return `
...
export function tplW0(input: BaseInput) {
  const nIt = nameOrDefault(input.name, "it");
  const nEn = nameOrDefault(input.name, "en");
  ...
  const itBody = [
    ...
    paragraph(
      ...
      `&bull; <strong>AVI</strong> — analisi multi-LLM su 8 prompt branded`,
    ),
    emailButton(withUtm(URL_AUDIT, "W0"), "Avvia il tuo primo audit"),
    paragraph(
      `In alternativa puoi partire da AVI (8 prompt gratuiti): <a href="${escapeHtml(withUtm(URL_AVI, "W0"))}">avi.citationrate.com</a>.`,
    ),
    paragraph(`Se ti serve qualsiasi cosa, rispondi a questa mail.`),
  ].join("");

  const enBody = [
    paragraph(`Hi ${escapeHtml(nEn)},`),
    ...
    paragraph(
      ...
      `&bull; <strong>AVI</strong> — multi-LLM analysis on 8 branded prompts`,
    ),
    emailButton(withUtm(URL_AUDIT, "W0"), "Start your free audit"),
    paragraph(
      `Or start with AVI (8 free prompts): <a href="${escapeHtml(withUtm(URL_AVI, "W0"))}">avi.citationrate.com</a>.`,
    ),
  ...
  return {
    ...
    html: emailLayout({
      lang: "it",
      preview: `Tutto pronto: 1 audit + 8 prompt AVI in 30 secondi.`,
...
export function tpl1A(input: BaseInput & { daysSinceSignup: number }) {
  ...
  const itBody = [
    ...
    paragraph(
      `Se preferisci AVI (analisi multi-LLM su prompt branded), hai 8 prompt gratuiti pronti: <a href="${escapeHtml(withUtm(URL_AVI, "1A"))}">Apri AVI</a>.`,
    ),
  ].join("");
  ...
  const enBody = [
    ...
    paragraph(
      `If you'd rather try AVI (multi-LLM analysis on branded prompts), you have 8 free prompts ready: <a href="${escapeHtml(withUtm(URL_AVI, "1A"))}">Open AVI</a>.`,
    ),
  ].join("");

  return {
    subject: `Your free audit is waiting | Hai 1 audit + 8 prompt AVI in attesa`,
    html: emailLayout({
      lang: "it",
...
export function tpl1B(input: BaseInput & { brand: string; scores: AuditScores; sector?: string }) {
  ...
  const zoneIt = scoreZone(scores.global, "it");
  const zoneEn = scoreZone(scores.global, "en");
  ...
  const itBody = [
    ...
    statTable([
      ["Score globale", `<strong>${Math.round(scores.global)}/100</strong> (${zoneIt})`],
      ...engineRows,
    ]),
    paragraph(
  ...
  const enBody = [
    ...
    statTable([
      ["Global score", `<strong>${Math.round(scores.global)}/100</strong> (${zoneEn})`],
      ...engineRows,
    ]),
    paragraph(
...
export function tpl1C(input: BaseInput & { brand: string; country?: string | null; result: AviResult }) {
  ...
  const itBody = [
    ...
    paragraph(`La demo gratuita usa 8 prompt no-browsing fissi. Con il piano Base hai a disposizione:`),
    paragraph(
...
export function tplD1(input: BaseInput) {
  ...
  const itBody = [
    paragraph(`Ciao ${escapeHtml(nIt)},`),
    paragraph(
      `ieri ti sei iscritto a CitationRate. Ti scriviamo solo per ricordarti che hai 1 audit + 8 prompt AVI gratuiti, pronti per essere usati.`,
    ),
  ...
  return {
    ...
    html: emailLayout({
      lang: "it",
      preview: `1 audit + 8 prompt AVI gratuiti, in 30 secondi.`,
      bodyInner: itBody + bilingualDivider() + enBody,
    }),
  };
}
...
export function tplD2(input: BaseInput & { sector?: string; city?: string }) {
  const nIt = nameOrDefault(input.name, "it");
  const nEn = nameOrDefault(input.name, "en");
  const sectorIt = input.sector || "il tuo settore";
  const sectorEn = input.sector || "your industry";
  const cityIt = input.city || "Italia";
  const cityEn = input.city || "your area";

  const itBody = [
    paragraph(`Ciao ${escapeHtml(nIt)},`),
    paragraph(
      `prova a chiedere a ChatGPT <em>"Quali sono i migliori ${escapeHtml(sectorIt)} in ${escapeHtml(cityIt)}?"</em>. La tua azienda compare nella risposta?`,
    ),
    ...
    paragraph(`Hai 1 audit + 8 prompt gratuiti in attesa:`),
  ...
  const enBody = [
    paragraph(`Hi ${escapeHtml(nEn)},`),
    paragraph(
      `try asking ChatGPT <em>"Who are the best ${escapeHtml(sectorEn)} in ${escapeHtml(cityEn)}?"</em>. Does your business appear in the answer?`,
    ),
...
export function tplD3(input: BaseInput) {
  ...
  const itBody = [
    ...
    paragraph(`Il tuo audit gratuito (1 CS + 8 prompt AVI) resta in attesa senza scadenza:`),
...
export function tplD4_AVI(input: BaseInput & { brand: string; projectId: string; runId: string; result: AviResult }) {
  ...
  const itBody = [
    ...
    statTable([
      ["AVI", `<strong>${Math.round(result.aviScore)}/100</strong>`],
      ["Presenza", `${presenceRound}% sugli 8 prompt branded`],
      ["Sentiment", `${Math.round(result.sentiment)}/100`],
...
export function tplD5_AVI(input: BaseInput & { brand: string; aviScore: number }) {
  ...
  const itBody = [
    ...
    paragraph(
      `Gli 8 prompt della demo sono no-browsing fissi, ti fornisce una situazione generale. Per vedere come i motori AI ti cercano DAVVERO (con web search live), per confrontarti con i competitor e per usare modelli in parallelo scopri Base.`,
    ),
    paragraph(
  ...
  return {
    subject: `What's beyond the 8 prompts | Cosa c'è oltre gli 8 prompt`,
    html: emailLayout({
      lang: "it",

...
export function AnalysisLauncher({
  ...
  // Demo plan: 1 run fisso. Garantisce coerenza con il cap server-side
  // se l'utente apre il modal con runCount > 1 (stato persistito).
  useEffect(() => {
    if (isDemo && runCount !== 1) setRunCount(1);
  }, [isDemo, runCount]);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative card p-8 w-full max-w-md text-center space-y-5 shadow-xl">
            ...
            <div className="flex gap-3">
              ...
              <a href="/piano#piani" className="flex-1 flex items-center justify-center gap-2 bg-[#c4a882] text-background font-semibold text-sm py-2.5 rounded-sm hover:bg-[#c4a882]/85 transition-colors">
                Scegli un piano
              </a>
            </div>
          </div>
      ...
      {open && !demoExhausted && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative card p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto space-y-5 shadow-xl">
            ...
            {/* Run count selector — hidden for demo (locked at 1 run) */}
            {!isDemo && (
              <div className="space-y-2">

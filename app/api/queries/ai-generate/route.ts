...
export async function POST(request: Request) {
  try {
    ...
    // Demo plan: cap a 2 query (fisse, generate da /demo-setup auto-flow).
    // 2 query × 4 motori = 8 prompts (costo ~$0.024 per demo).
    const { data: profileRow } = await (supabase.from("profiles") as any)
      .select("plan")
      .eq("id", user.id)
      .single();
    const userPlan = (profileRow as any)?.plan ?? "demo";
    const count = userPlan === "demo" ? Math.min(requestedCount, 2) : requestedCount;
...
✅ GIUSTO: "Quali studi o società di consulenza fiscale aiutano le PMI italiane a ridurre le tasse legalmente?"`;
  ...
  if (isSpecific && userInputs?.theme_context) userContextParts.push(`- Obiettivo dell'indagine (PUNTO DI VISTA da cui generare le query): ${sanitizeInput(userInputs.theme_context, 4000)}`);
  ...
  if (userInputs?.punti_di_forza?.length) userContextParts.push(`- Caratteristiche distintive del brand: ${sanitizeArray(userInputs.punti_di_forza).join(", ")}`);
  if (userInputs?.obiezioni?.length) userContextParts.push(`- Considerazioni dei clienti: ${sanitizeArray(userInputs.obiezioni).join(", ")}`);
